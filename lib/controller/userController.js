...
Meteor.methods({
  ...
  'editRole':function(eidd,fname,lname,email,tel,role,department,specialize){
  ...
  'viewDoctorInfo_search':function(doctorID){
    console.log('find doctor :'+doctorID);
    user=User.findOne({eid:doctorID},{});
    return user;
  },
  'removePatient':function(hn){
    user=User.findOne({hn:hn});
    if(user!=null){
      appts = Appointment.remove({patient_hn:hn});
  ...
  'removeEmployee':function(eid){
    user=User.findOne({eid:eid});
    if(user!=null){
      appts = Appointment.remove({doctor_eid:eid});
      schedules = DoctorSchedule.remove({eid:eid});
      User.remove(user._id);
      return true;
    }
    return false;
